...
function HaloIora(params) {
    ...
    async function load_lights() {
        ...
        const point1 = new THREE.AmbientLight(0xffffff, 0.4, 10000);
        ...
        // Light w/o reflection
        const point2 = new THREE.AmbientLight(0xffffff, 0.4, 10000);
    ...
    async function load_cameras() {
        ...
        // 2D
        function ioraResponsive(mobile, tablet) {
            if (mobile.matches) {
                camera = new THREE.OrthographicCamera(
                    $container.width() / -1.25,
                    $container.width() / 1.25,
                    $container.height() / 1.25,
                    $container.height() / -1.25,
                    0.1,
                    10000
                );
        ...
        var mobile = window.matchMedia("(max-width: 541px)");
        var tablet = window.matchMedia("(max-width: 913px)");
        ioraResponsive(mobile, tablet);
        mobile.addListener(ioraResponsive);
        tablet.addListener(ioraResponsive);
    ...
    async function load_objects() {
        ...
        const gltf = await new Promise((res, rej) =>
            loader.load("./assets/2d/halo-iora.glb", res, undefined, rej)
        );
